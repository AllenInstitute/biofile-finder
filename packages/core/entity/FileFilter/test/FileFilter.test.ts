...
describe("FileFilter", () => {
    ...
    describe("toSQLWhereStringForNestedSiblings", () => {
        it("returns a trivially-true clause for an empty filter list", () => {
            expect(FileFilter.toSQLWhereStringForNestedSiblings([])).to.equal("1=1");
        });
        ...
        it("ANDs conditions on different sub-fields inside a single list_filter lambda", () => {
            ...
            expect(FileFilter.toSQLWhereStringForNestedSiblings(filters)).to.equal(
