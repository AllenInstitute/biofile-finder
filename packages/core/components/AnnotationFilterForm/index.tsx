 ...
 *    Uses `FileFilter` with `nestedParent` which generates a `list_transform` query.
...
export default function AnnotationFilterForm(props: AnnotationFilterFormProps) {
    ...
    const createFileFilter = (item: ListItem) => {
        ...
        return new FileFilter(props.annotation.path, value, filterType);
    };
    ...
    function onApplyByField() {
        ...
        const subFieldPath = [...props.annotation.path, ...segments];
        const subFieldName = subFieldPath.join(".");
        dispatch(
            selection.actions.setFileFilters([
                ...allFilters.filter((f) => f.name !== subFieldName),
                new FileFilter(subFieldPath, "", FilterType.ANY),
            ])
        );
    }
    ...
    function onApplyByFieldValue(value: string) {
        ...
        dispatch(
            selection.actions.addFileFilter(
                new FileFilter(subFieldPath, value, FilterType.DEFAULT)
            )
        );
    }
            ...
            case AnnotationType.NUMBER:
                return (
                    <NumberRangePicker
                        ...
                        // TODO: displayValue is ignored here bc it became a ReactNode...
                        items={items.map(i => ({ displayValue: i.value, value: i.value }))}
                        loading={isLoading}
                        errorMessage={errorMessage}
                        onSearch={onSearch}
                        currentRange={filtersForAnnotation?.[0]}
